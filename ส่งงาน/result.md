...
### เพิ่ม Build Information (ใน Docker Container)
<img width="502" height="530" alt="image" src="https://github.com/user-attachments/assets/225fdbf4-bafd-4c23-95a6-4ceb13a6523a" />

### คำถามทบทวน
1.Docker vs Native Setup: อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host system
ตอบ การใช้ Docker กับ ESP-IDF ช่วยให้ติดตั้งง่าย แยกสภาพแวดล้อมจากระบบหลัก และลดปัญหาความเข้ากันของเวอร์ชัน ขณะที่การติดตั้งบน Host System เหมาะสำหรับการ debug และเชื่อมต่อฮาร์ดแวร์โดยตรงได้สะดวกกว่า

2.Build Process: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
ตอบ ขั้นตอนการ build ESP-IDF ใน Docker เริ่มจาก mount โฟลเดอร์โปรเจกต์เข้า container แล้วรันคำสั่ง idf.py build หลังจาก export สภาพแวดล้อม ซึ่งจะได้ไฟล์ binary อยู่ในโฟลเดอร์ build/ พร้อมใช้งาน

3.CMake Files: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
ตอบ ไฟล์ CMakeLists.txt ใช้กำหนดโครงสร้างโปรเจกต์และบอก CMake ว่าต้อง build อะไร โดย CMakeLists.txt ระดับบนสุดควบคุมโปรเจกต์หลัก ส่วนในแต่ละ component จะควบคุมเฉพาะโมดูลนั้น ซึ่ง Docker จะเรียกใช้ CMake ผ่าน idf.py build เพื่อจัดการ build ทั้งหมดแบบอัตโนมัติ

4.Git Ignore: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
ตอบ ไฟล์ .gitignore มีหน้าที่กันไม่ให้ไฟล์ชั่วคราว เช่น build/, .pio/, หรือไฟล์ binary ถูก push ขึ้น Git ซึ่งช่วยให้โปรเจกต์สะอาด เป็นระเบียบ และลดปัญหา merge conflict ในการพัฒนา ESP32 ร่วมกัน

5.Container Persistence: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
ตอบ เมื่อ restart container ข้อมูลภายใน container ที่ไม่ถูก mount เช่น ไฟล์ชั่วคราวหรือการเปลี่ยนแปลงระบบจะหายไป แต่ข้อมูลที่อยู่บน volume หรือที่ mount จาก host เช่น source code หรือ build files จะยังอยู่เหมือนเดิม

6.Development Workflow: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
ตอบ การพัฒนาบน Docker มี workflow ที่ยืดหยุ่น ติดตั้งง่าย และให้สภาพแวดล้อมที่เสถียรเท่ากันทุกเครื่อง ส่วนการพัฒนาบน native system อาจต้อง config เองมากกว่า แต่เข้าถึงอุปกรณ์และ debug ได้สะดวกกว่า

### Lab6_2

### คำถามทบทวน
1.Multiple Source Files: เหตุใดต้องแยก source code เป็นหลายไฟล์?
ตอบ การแยก source code เป็นหลายไฟล์ช่วยให้โค้ดอ่านง่าย ดูแลรักษาสะดวก และส่งเสริมการทำงานเป็นทีม โดยสามารถแบ่งหน้าที่ตามโมดูลหรือฟังก์ชันเฉพาะ ทำให้แก้ไขหรือพัฒนาแต่ละส่วนได้โดยไม่กระทบกัน

2.CMakeLists.txt Management: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
ตอบ การเพิ่มไฟล์ source ใหม่ในโปรเจกต์ ESP-IDF ต้องสร้างไฟล์ .c และอาจเพิ่มไฟล์ .h ประกอบ พร้อมแก้ CMakeLists.txt เพื่อระบุไฟล์ใหม่นั้นเข้าไปในขั้นตอน build

3.Header Files: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
ตอบ ไฟล์ .h ทำหน้าที่ประกาศฟังก์ชัน ตัวแปร และโครงสร้างข้อมูลเพื่อให้ไฟล์อื่นเรียกใช้งานได้อย่างถูกต้อง ช่วยแยกส่วนประกาศกับการทำงานจริงและส่งเสริมการเขียนโค้ดที่เป็นระบบและอ่านง่าย

4.Include Directories: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
ตอบ INCLUDE_DIRS ใน CMakeLists.txt ระบุโฟลเดอร์ที่เก็บไฟล์ header เพื่อให้ compiler รู้ว่าจะหาไฟล์ .h จากที่ไหน ช่วยให้การเชื่อมโยงโค้ดระหว่างไฟล์ต่าง ๆ ถูกต้องและไม่มีข้อผิดพลาดตอนคอมไพล์

5.Git Ignore: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
ตอบ ไฟล์ .gitignore ช่วยกรองไฟล์และโฟลเดอร์ที่ไม่จำเป็นหรือสร้างขึ้นอัตโนมัติ เช่น ไฟล์ build และ binary ออกจากการติดตามของ Git ทำให้ repository สะอาดและลดปัญหาการแชร์ไฟล์ขยะในทีม

6.Task Management: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
ตอบ การใช้ FreeRTOS task ในโมดูล LED ช่วยให้การควบคุมไฟ LED ทำงานแยกจากงานหลัก ทำให้ระบบตอบสนองรวดเร็วและสามารถจัดการหลายงานพร้อมกันอย่างมีประสิทธิภาพ

7.Code Organization: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
ตอบ การแยกโมดูล sensor, display, และ led เป็นไฟล์แยกช่วยเพิ่มความเป็นระเบียบ ทำให้ง่ายต่อการพัฒนา แก้ไข และทดสอบแยกส่วนโดยไม่กระทบกันในโปรเจกต์
